...
DECLARE
  ...
  v_reuniao int := 0;
BEGIN
  -- Tarefas: somente finalizadas (não usa mais data_prevista vencida)
  UPDATE public.todo
     SET responsaveis_ids = '{}'::uuid[],
         responsavel_id = NULL,
         equipe_toda = false,
         em_teste = false
   WHERE status IN ('encerrada','concluida','producao')
...
REVOKE EXECUTE ON FUNCTION public.limpar_atribuicoes_finalizadas() FROM PUBLIC, anon, authenticated;
