...
DECLARE
  ...
  v_users UUID[];
BEGIN
  -- Tarefas com prazo em até 24h (não concluídas)
  FOR r IN
  ...
  LOOP
    ...
    FOREACH v_user IN ARRAY (SELECT array_agg(DISTINCT u) FROM unnest(v_users) u WHERE u IS NOT NULL) LOOP
      -- evita duplicar: só insere se não existe notificação do tipo no último dia para essa tarefa
      IF NOT EXISTS (
  ...
  -- Chamados externos com prazo estourado e ainda abertos
  FOR r IN
...
REVOKE EXECUTE ON FUNCTION public.gerar_notificacoes_prazo() FROM PUBLIC, anon, authenticated;

-- Agenda a cada 15 minutos
DO $$
BEGIN
  PERFORM cron.unschedule('gerar-notificacoes-prazo');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'gerar-notificacoes-prazo',
  '*/15 * * * *',
  $$ SELECT public.gerar_notificacoes_prazo(); $$
);